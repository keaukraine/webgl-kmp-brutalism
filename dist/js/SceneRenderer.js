    ...
    async loadSceneTextures() {
        ...
        for (const texture of textures) {
            ...
            if (formatName === "RGBA8" || formatName === "RGB8") {
                glTexture = await webgl_framework_1.UncompressedTextureLoader.load(`data/textures/${texture.fileName}.webp`, // WEBP only
                ...
                glTexture = await webgl_framework_1.UncompressedTextureLoader.load(`data/textures/${texture.fileName}.webp`, // WEBP instead of ASTC
                gl, minFilter, magFilter, clamp);
                if (texture.mipmaps > 0) {
                    gl.bindTexture(gl.TEXTURE_2D, glTexture);
                    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.LINEAR);
                ...
                glTexture = (0, TextureUtils_1.loadFloatingPointTexture)(this, data, gl, texture.width, texture.height, minFilter, magFilter, clamp, "fp16", 3);
                this.textures.push(glTexture);
                ...
                const fileName = `data/textures/${texture.fileName}`;
                const data = await (0, FetchUtils_1.fetchArrayBuffer)(fileName);
                glTexture = (0, TextureUtils_1.loadFloatingPointTexture)(this, data, gl, texture.width, texture.height, minFilter, magFilter, clamp, "fp16", 4);
                this.textures.push(glTexture);
                console.log(`Loaded texture ${fileName} [${texture.width}x${texture.height}]`);
            }
                ...
                const fileName = `data/textures/${texture.fileName}`;
                const data = await (0, FetchUtils_1.fetchArrayBuffer)(fileName);
                glTexture = (0, TextureUtils_1.loadFloatingPointTexture)(this, data, gl, texture.width, texture.height, minFilter, magFilter, clamp, "fp32", 3);
                this.textures.push(glTexture);
                console.log(`Loaded texture ${fileName} [${texture.width}x${texture.height}]`);
            }
        }
