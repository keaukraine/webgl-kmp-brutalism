...
import { loadFloatingPointTexture } from "./utils/TextureUtils";
    ...
    protected async loadSceneTextures(): Promise<void> {
        ...
        for (const texture of textures) {
            ...
            if (formatName === "RGBA8" || formatName === "RGB8") {
                glTexture = await UncompressedTextureLoader.load(
                    `data/textures/${texture.fileName}.webp`, // WEBP only
                    gl,
                    minFilter,
                    magFilter,
                    clamp
                ...
                glTexture = await UncompressedTextureLoader.load(
                    `data/textures/${texture.fileName}.webp`, // WEBP instead of ASTC
                    gl,
                    minFilter,
                    magFilter,
                    clamp
                ...
                glTexture = loadFloatingPointTexture(
                    this,
                    data,
                    gl,
                    texture.width,
